# Mejora o evolución del sistema

## Índice

- [Introducción](#introducción)
- [Comparativa entre las configuraciones propuestas](#comparativa-entre-las-configuraciones-propuestas)
- [Posibles mejoras de hardware](#posibles-mejoras-de-hardware)
- [Evolución del sistema si la empresa crece](#evolución-del-sistema-si-la-empresa-crece)
- [Valoración final](#valoración-final)

## Introducción

La infraestructura hardware propuesta se ha diseñado para cubrir las necesidades actuales del club, manteniendo un equilibrio entre rendimiento, coste, estabilidad y facilidad de mantenimiento. Sin embargo, como ocurre en cualquier sistema informático, esta infraestructura no debe considerarse cerrada o definitiva, sino preparada para evolucionar en función del crecimiento de la organización y de los cambios en sus necesidades.

Por este motivo, resulta importante realizar una pequeña reflexión sobre las posibles mejoras del hardware, las diferencias entre las configuraciones planteadas y la forma en la que el sistema podría ampliarse en el futuro.

## Comparativa entre las configuraciones propuestas

Uno de los aspectos más claros de la infraestructura es que no todos los puestos de trabajo requieren el mismo nivel de potencia. Por ello, se han planteado distintas configuraciones adaptadas al perfil de cada usuario.

El **equipo estándar** cubre correctamente las necesidades de la mayoría de departamentos, como Dirección y Finanzas, Recepción, Tienda oficial o parte del Cuerpo técnico. Se trata de una configuración equilibrada, suficiente para tareas de oficina, gestión documental, correo electrónico, navegación web y uso de aplicaciones corporativas. Su principal ventaja es que ofrece un buen rendimiento sin elevar demasiado el coste de cada puesto.

El **equipo estándar mejorado**, asignado al área de Prensa y Marketing, representa un punto intermedio entre un ordenador corporativo convencional y una estación de trabajo más potente. Gracias a su mejor procesador y a la ampliación de memoria RAM, ofrece una mayor capacidad para multitarea, edición ligera y trabajo visual. Esto permite adaptarse mejor a tareas más exigentes sin llegar al coste de una workstation profesional.

La **HP Z4 G5**, destinada a Soporte IT, supone un salto importante en robustez y capacidad de ampliación. Frente a un equipo estándar, ofrece una plataforma más profesional, mayor estabilidad y un hardware más adecuado para tareas técnicas avanzadas, virtualización, pruebas, administración de sistemas o análisis. Su principal diferencia no es solo la potencia, sino también la fiabilidad y la orientación profesional de sus componentes.

Por último, el **equipo de alto rendimiento diseñado a medida** representa la opción más potente de toda la infraestructura. Su gran capacidad de memoria, la tarjeta gráfica dedicada de gama alta y el almacenamiento rápido lo convierten en una solución claramente superior para tareas creativas exigentes. Frente al equipo estándar mejorado, ofrece una mayor capacidad gráfica y un margen mucho más amplio para edición multimedia, diseño avanzado o creación de contenido.

## Posibles mejoras de hardware

Aunque la infraestructura propuesta es adecuada para las necesidades actuales del proyecto, existen varias mejoras que podrían aplicarse en el futuro.

Una de las primeras mejoras posibles sería la **ampliación de memoria RAM** en algunos puestos. Por ejemplo, los equipos estándar podrían evolucionar de 16 GB a 32 GB si aumentara el uso de aplicaciones más pesadas o si la empresa adoptara herramientas de trabajo más exigentes. Esta mejora permitiría alargar la vida útil de los equipos sin necesidad de sustituirlos completamente. Sin embargo, la actual crisis en la que los precios de la RAM han aumentado de manera considerable deja este escenario un poco en el aire.

Otra mejora clara sería el **aumento de la capacidad de almacenamiento SSD** en determinados puestos, especialmente en los departamentos de Prensa y Marketing y en Soporte IT. A medida que crezcan los archivos de trabajo, el material gráfico, las imágenes de sistema o las herramientas técnicas, disponer de más capacidad local puede resultar necesario.

En el caso del servidor, una mejora importante sería sustituir en el futuro los discos duros por unidades de mayor capacidad o incluso combinar almacenamiento HDD y SSD según el tipo de datos. También podría plantearse una solución de almacenamiento más avanzada, con mejor rendimiento o mayor tolerancia a fallos, si la empresa dependiera cada vez más de los servicios centralizados.

Otra posible mejora estaría relacionada con la **tarjeta gráfica**. Actualmente, solo el equipo a medida dispone de una GPU potente, algo lógico por su función específica. Sin embargo, si el departamento de Prensa y Marketing aumentara su carga de trabajo multimedia, podría ser conveniente incorporar gráficas dedicadas también en alguno de los equipos superiores.

## Evolución del sistema si la empresa crece

Si el club creciera en número de empleados, departamentos o volumen de trabajo, la infraestructura hardware debería evolucionar en consecuencia.

En primer lugar, sería necesario aumentar el número de **puestos estándar** para cubrir nuevas incorporaciones. Este crecimiento sería relativamente sencillo, ya que el equipo estándar está planteado como una solución equilibrada y fácil de replicar dentro del sistema.

También podría ser necesario ampliar el número de **equipos de gama superior** en departamentos donde aumentara la carga de trabajo creativo o técnico. Por ejemplo, si el área de Prensa y Marketing creciera, lo más lógico sería añadir más puestos con configuración mejorada e incluso más de un equipo de alto rendimiento.

En Soporte IT, un crecimiento de la infraestructura también implicaría una mayor exigencia técnica. En ese caso, las workstations podrían ampliarse en memoria, almacenamiento o incluso complementarse con nuevos equipos técnicos especializados.

A nivel de almacenamiento, el crecimiento de la empresa implicaría casi con seguridad una mayor necesidad de espacio en el servidor principal. Esto obligaría a replantear la capacidad de los discos, el sistema RAID e incluso la estrategia general de almacenamiento y copias de seguridad.

Del mismo modo, si la infraestructura de red se volviera más compleja, también podría ser necesario reforzar los equipos de administración técnica, las herramientas de monitorización y la capacidad del servidor central.

## Valoración final

En conjunto, la infraestructura propuesta resulta adecuada para el tamaño actual del proyecto y responde de forma coherente a las necesidades de cada departamento. La existencia de distintos niveles de hardware permite optimizar recursos, evitando asignar equipos excesivamente potentes a puestos que no los necesitan y reservando las configuraciones más avanzadas para los perfiles que realmente las aprovechan.

Al mismo tiempo, el sistema mantiene un margen razonable de evolución. La posibilidad de ampliar memoria, almacenamiento o incluso incorporar hardware más potente en determinados puestos demuestra que la infraestructura no solo está pensada para funcionar en el presente, sino también para adaptarse a cambios futuros.

Por tanto, esta propuesta de hardware no debe entenderse como una solución rígida, sino como una base técnica escalable, equilibrada y preparada para crecer junto con la organización.